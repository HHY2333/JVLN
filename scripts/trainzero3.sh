...
OUTPUT_DIR="/data/model/JanusVLN_Base/zero3"
CACHE_DIR="./cache"
mkdir -p $OUTPUT_DIR
...
# ==========================================
# ZeRO-2 + CPU offload + DeepSpeed C++ Adam
# 硬件约束分析（A800 PCIe，无 NVLink，2卡）：
#   - ZeRO-3: 通信量过大，跨NUMA PCIe带宽不够，直接卡死
#   - ZeRO-2 纯GPU: optimizer fp32 states 28GB/卡 → 70GB超限 OOM
#   - 唯一可行: CPU offload optimizer
# 速度优化关键：在DS配置中指定 "type": "Adam"
#   DeepSpeed 自动切换为 C++ DeepSpeedCPUAdam（比 PyTorch CPU AdamW 快5-10x）
#   GPU 每卡只需: 参数14GB + 梯度7GB + 激活~5GB ≈ 26GB，大量余量
# ==========================================
cat > /tmp/zero2_cpu_adam.json << 'EOF'
{
  "bf16": {
    "enabled": true
  },
...
torchrun \
    ...
    --save_steps 200 \
    --save_total_limit 4 \
    --deepspeed /tmp/zero2_cpu_adam.json \
